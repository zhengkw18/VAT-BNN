...
def train_epoch(model, labeled_train_loader, unlabeled_train_loader, optimizer, device, epsilon):  # Training Process
    ...
    for i, (input, target) in enumerate(labeled_train_loader):
        ...
        if unlabeled_iter is not None:
            ...
            unlabeled_input = unlabeled_input.to(device)
...
if __name__ == "__main__":
    ...
    config = 'vat_batch-{}_epochs-{}_dataset-{}_labelnum-{}_epsilon-{}'.format(args.batch_size, args.epochs, args.dataset, args.label_num, args.epsilon)
    ...
    if args.do_train:
        ...
        for epoch in range(args.epochs):
            print(f"Epoch: {epoch}")
            train_loss, train_acc = train_epoch(model, labeled_train_loader, unlabeled_train_loader, optimizer, device, args.epsilon)
